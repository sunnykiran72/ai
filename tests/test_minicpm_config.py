...
class TestMiniCPMConfigDefaults(unittest.TestCase):
    ...
    def test_from_env_defaults(self):
        ...
        with patch.dict(os.environ, {}, clear=True):
            ...
            self.assertIn("return exactly one valid json object", garment_prompt)
            self.assertIn("category_type", garment_prompt)
            self.assertIn("garment_construction_prompt", garment_prompt)
            self.assertIn("output only json", garment_prompt)
            self.assertIn("no directional/side terms", garment_prompt)
...
class TestMiniCPMConfigEnvironmentVariables(unittest.TestCase):
    ...
    def test_category_prompt_helpers_are_rich_and_type_specific(self):
        ...
        self.assertIn("describe only the top garment", top_prompt)
        self.assertIn("no non-top garment or hardware details", top_prompt)
        self.assertIn("category_type", top_prompt)

        self.assertIn("describe only the bottom garment", bottom_prompt)
        self.assertIn("waistband/rise/opening shape", bottom_prompt)
        self.assertIn("no non-bottom garment or hardware details", bottom_prompt)

        self.assertIn("describe only the dress garment", dress_prompt)
        self.assertIn("bodice-to-skirt continuity as one garment", dress_prompt)
        self.assertIn("no non-dress garment or hardware details", dress_prompt)

        self.assertIn("describe only the outer garment", outer_prompt)
        self.assertIn("no non-outer garment or hardware details", outer_prompt)
        self.assertIn("if upper edge is broad/pleated/draped, use panel/band wording", outer_prompt)
